...
let card = document.getElementById("card-temp")//se llama al Id del div de card para añadirla
let createCheckBox = document.getElementById("checkbox")//toma un formulario desde un ID para añadir un checkbox
let elementEvents = data.events//llama al objeto "events" dentro del array data
let buscador = document.getElementById("inputSearch")//se llama al al imput texto para volverlo un buscador
...
function crearCheckbox(datos, cardsitas){//crea los checkbox
    ...
    <div class="form-check form-check-inline">
        <input class="form-check-input" type="checkbox" value="${x}" id="${x}">
...
function crearCard(datos, contenedor){//crea las cards
     ...
     <div class="card">
         <img src="${x.img}" class="card-img-top" height=170px alt="...">
           <div class="card-body">
               ...
               <a id="linC" href="./details.html?id=${x._id}" class="btn btn-primary">Go somewhere</a>
            </div>
       </div>
   </div> `;});    
};
 ...
 function filter(event) {//filtrador por categoria
  ...
  function filterText(eventos, value){
    return eventos.filter((event) =>
    event.name.toUpperCase().includes(value.toUpperCase())
);
}
  ...
  function filtrar(eventos){
    console.log (buscador.value)
    let filterOfText = filter(eventos)
    let filterOfBusq = filterOfBusq(buscador.value)
    return filterOfBusq
  }